...
Master 在每个子任务执行期间启动 VisionMonitor 线程，利用顶视 USB 摄像头 + GPT-4o 视觉能力进行主动监控：

```
子任务下发 → 启动监控线程
               │
               ├── 每3秒：抓取帧 → base64 → GPT-4o 判断
               │   返回：{"status": "executing|completed|failed",
...
**成功/失败检测：** 检查 MCP 返回字符串是否包含 `"FAILED"` 关键字（与 RoboSkill 的 `skill_failure()` 约定一致）。
